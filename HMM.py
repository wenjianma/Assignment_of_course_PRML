...
class HMM:
    ...
    def rand(self, nSamples):
        ...
        # get a random state sequence from the Markov Chain
        S = self.stateGen.rand(nSamples)
        
        # get output data samples from different distributions according to the state
        X = []
        for i in S:
            xi = self.outputDistr[i].rand(1)[0][0]
            X.append(xi)
        X = np.array(X)
        S = np.array(S)
        return [X,S]
