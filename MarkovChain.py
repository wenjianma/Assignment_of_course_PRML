...
class MarkovChain:
    ...
    def rand(self, tmax):
        ...
        s1 = rng.choice(self.nStates, p=self.q)
        S = [s1]
        s_prev = s1
        
        if self.is_finite == False:
            while len(S) < tmax:
                s_new = rng.choice(self.nStates, p=self.A[s_prev])
                S.append(s_new)
                s_prev = s_new
        else:
            while len(S) < tmax:
                s_new = rng.choice(self.A.shape[1], p=self.A[s_prev])
                if s_new == self.A.shape[1]-1:
                    break
                else:
                    S.append(s_new)
                    s_prev = s_new
